...
class AdvancedDetectionService {
  static final AdvancedDetectionService _instance = AdvancedDetectionService._internal();
  ...
  Future<void> initialize() async {
    ...
    _interpreter = await Interpreter.fromAsset('assets/models/question_detector.tflite');
  }
  ...
  Future<List<Point<int>>?> _detectDocumentCorners(img.Image image) async {
    ...
    try {
      ...
      return corners.map((corner) => Point(corner.x.toInt(), corner.y.toInt())).toList();
    } catch (e) {
      print('文档边角检测失败: $e');
      return null;
    }
  ...
  Future<List<List<double>>> _runInference(img.Image image) async {
    ...
    // 运行模型推理
    final outputBuffer = List<double>.filled(100 * 4, 0); // 假设最多检测100个区域，每个区域4个坐标
  ...
  List<TextBlock> _analyzeStructure(RecognizedText recognizedText, List<List<double>> modelOutput) {
  ...
  bool _hasTypicalQuestionStructure(TextBlock block) {
    ...
    final hasOptions = lines.any((line) => 
      RegExp(r'^[A-D][\.\、\)]').hasMatch(line.trim())
    );
    
    // 检查段落缩进
    final hasIndentation = lines.any((line) => 
  ...
  void _validateWithModelOutput(List<TextBlock> blocks, List<List<double>> modelOutput) {
    // 使用模型输出结果验证和调整文本块
    for (var output in modelOutput) {
      ...
      for (var block in blocks) {
        final intersection = _calculateIntersection(block.boundingBox, modelRect);
        if (intersection > 0.7) { // 如果重叠度超过70%
          // 调整文本块边界以更好地匹配模型预测
  ...
  double _calculateIntersection(Rect r1, Rect r2) {
    ...
    final unionArea = r1.width * r1.height + r2.width * r2.height -
  ...
  bool _areBlocksRelated(TextBlock b1, TextBlock b2) {
    ...
    final horizontalOverlap = math.max(0,
