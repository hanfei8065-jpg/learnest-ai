...
class QuestionDetectionService {
  static final QuestionDetectionService _instance = QuestionDetectionService._internal();
  ...
  Future<Rect?> detectQuestionArea(File imageFile) async {
    try {
      ...
      final surroundingBlocks = _findRelatedBlocks(questionBlock, recognizedText.blocks);
  ...
  Rect? _findQuestionByLayout(List<TextBlock> blocks, Document document) {
    ...
    for (var i = 0; i < sortedBlocks.length - 1; i++) {
      ...
      final nextBlocks = sortedBlocks.sublist(i + 1)
        .takeWhile((b) => b.boundingBox.top - currentBlock.boundingBox.bottom < 100)
        .toList();

      if (nextBlocks.length >= 2) { // 至少要有题干和两个选项
  ...
  List<TextBlock> _findRelatedBlocks(TextBlock questionBlock, List<TextBlock> allBlocks) {
    ...
    for (var block in allBlocks) {
      ...
      if (block.boundingBox.top >= questionBlock.boundingBox.top &&
          ...
          (block.boundingBox.left - questionLeft).abs() < 50) { // 水平对齐度
