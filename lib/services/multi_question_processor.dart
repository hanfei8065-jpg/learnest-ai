...
class MultiQuestionProcessor {
  static final MultiQuestionProcessor _instance = MultiQuestionProcessor._internal();
  ...
  Future<List<Question>> processMultipleQuestions(File imageFile) async {
    ...
    try {
      // 1. 获取所有题目区域
      final questionAreas = await _advancedDetector.detectQuestionAreas(imageFile);
  ...
  String _estimateDifficulty(QuestionComponents components) {
    ...
    // 1. 基于题干长度
    if (components.stem.length > 200) score += 2;
    else if (components.stem.length > 100) score += 1;
